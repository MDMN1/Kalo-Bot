...
if (message.channel.type !== 'text') {
  ...
  let everyone = guild.roles.find(r => r.name === ('@'+'everyone'))
    ...
    channel = await guild.createChannel(`${message.author.username}`)
              ...
              await channel.setTopic(`«close to close the ticket for ${message.author.tag}`, 'Info for Staff to close ticket')
...
let supportUser = client.users.get(support.targetID);
  ...
  if  (message.content.toLowerCase() === '«close') {
    ...
    const embed = new Discord.RichEmbed()
          .setColor('#6d6d6d')
          .setAuthor(message.author.tag, message.author.displayAvatarURL)
          .setFooter(`Message Recieved -- Ralston Test Server`)
          .setDescription(message.content)
